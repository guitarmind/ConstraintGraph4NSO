...
def train(args, train_dataset, model, tokenizer):
    ...
    num_warmup_steps = int(t_total * args.warmup_steps_ratio)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {'input_ids': batch[0], 'attention_mask': batch[
                1], 'token_type_ids': batch[2], 'labels': batch[3]}
            ...
            if global_step % save_steps == 0:
                ...
                print('loss: ' + str((tr_loss - logging_loss) /
                                     save_steps) + ' step: ' + str(global_step))
...
def evaluate_test(args, model, tokenizer, file_type, prefix=""):
    ...
    cached_features_file = os.path.join(args.data_dir, 'cached_{}_{}_{}_{}'.format(
        file_type, 'codebert', str(args.max_seq_length), 'pair_order'))
    ...
    file_h = codecs.open(args.data_dir + file_type + "_results." +
                         str(args.window_size) + ".tsv", "w", "utf-8")
    ...
    for eval_output_dir in eval_outputs_dirs:
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            for widx in range(logits.shape[0]):
                outF.writerow([rows['guid'][widx], rows['text_a'][widx], rows['text_b'][widx], rows['labels'][
                              widx], rows['pos_a'][widx], rows['pos_b'][widx], logits[widx][0], logits[widx][1], tmp_pred[widx]])
            if preds is None:
                preds = logits
        ...
        output_eval_file = os.path.join(
            eval_output_dir, file_type + "_results.txt")
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    cached_features_file = os.path.join(
        args.data_dir, 'cached_{}_{}_{}_{}'.format('dev' if evaluate else 'train',
                                                   'codebert', str(args.max_seq_length), 'pair_order'))
...
def main():
    ...
    parser.add_argument("--warmup_steps_ratio", default=0.05, type=float,
                        help="Linear warmup ratio over warmup_steps.")
    ...
    if args.do_train:
        ...
        tokenizer = tokenizer_class.from_pretrained(
            args.output_dir, do_lower_case=False)
        ...
        for checkpoint in checkpoints:
            ...
            if args.do_test:
                result = evaluate_test(
                    args, model, tokenizer, "train", prefix=global_step)
                result = evaluate_test(
                    args, model, tokenizer, "val", prefix=global_step)
                result = evaluate_test(
                    args, model, tokenizer, "test", prefix=global_step)
            elif args.do_eval:
